...
simple_bind(SQL,Params) when is_list(SQL), is_list(Params) ->
  ...
  UTF8 = convert_codepoints(SQL),
  bind_params(UTF8,TaggedParams);
...
tag_type(#datetime{} = Date) ->
  Date.
%%=======================================================================  
convert_codepoints(Text) ->
  case lists:any(fun is_codepoint/1, Text) of
    true ->
...
%%=======================================================================  
is_codepoint(C) when C >= 0,
                     C =< 255 ->
  false;
is_codepoint(C) when C > 255 ->
  true.
